...
int main() {
    ...
    while(t--) {
        ...
        ll power_sum=0;
        ll weight_sum=0;
        rep(i, n) power_sum+=deer[i].first-deer[i].second;
        irep(k, n) {
            if(power_sum>=weight_sum) ans=k;
            if(k<n) {
                power_sum-=deer[n-1-k].first-deer[n-1-k].second;
                weight_sum+=deer[n-1-k].second;
            }
        }
        cout << ans << endl;
    }
